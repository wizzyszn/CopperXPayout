...
const loginWizard = new telegraf_1.Scenes.WizardScene("loginWizard", 
// Step 0: Email input
async (ctx) => {
...
async (ctx) => {
    ...
    try {
        ...
        const response = await (0, copperX_service_1.requestOtp)(input);
        if (!response.sid) {
            await ctx.reply("❌ Failed to generate verification code. Please try again later.");
            return ctx.scene.leave();
        }
        ctx.session.sid = sessionManager_1.sessionManager.EncryptData(response.sid); // Encrypt and store SID
        await ctx.reply("ℹ️ If this email is valid and active, you will receive a verification code (OTP). Please enter it now:");
...
async (ctx) => {
    ...
    const sid = sessionManager_1.sessionManager.DecryptData(ctx.session.sid);
    ...
    try {
        ...
        const response = await (0, copperX_service_1.authenticateOtp)({ otp, email, sid });
        if (response.accessToken) {
            sessionManager_1.sessionManager.setToken(ctx, response.accessToken);
            ctx.session.userInfo = response.user;
            await ctx.replyWithHTML(`🎊 Login successful!\n\n🚀 Welcome to CopperX Bot, ${response.user.email}!\n\nI'm here to help you manage your CopperX account. Choose an option below:`, {
                reply_markup: {
                    inline_keyboard: [
                        [
